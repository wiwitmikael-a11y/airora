...
const App: React.FC = () => {
    ...
    const handleGenerateVideo = useCallback(async (prompt: string, uploadedImage?: ChatMessage['uploadedImage'], onError?: (error: Error) => void) => {
    ...
    const handleUseImageForVideo = async (image: GeneratedImage) => {
        if (!image.imageUrl) return;
        playSound('open');
        const response = await fetch(image.imageUrl);
        const blob = await response.blob();
        ...
        reader.onloadend = () => {
            ...
            sessionStorage.setItem('airora_init_image_for_video', JSON.stringify({ url: dataUrl, mimeType: blob.type }));
            handleSelectView(ViewType.VIDEO);
        };
        reader.readAsDataURL(blob);
    };

    const handleUseVideoForStory = (prompt: string) => {
        handleSelectView(ViewType.POETIC);
        setTimeout(() => {
            handleSendMessage(`Write a poetic narrative inspired by this scene: "${prompt}"`);
                    ...
                    <>
                        ...
                        {activeView === ViewType.IMAGE && (
                            <ImageView
                                ...
                                onUseImageForVideo={handleUseImageForVideo}
                            />
                        )}
                        {activeView === ViewType.VIDEO && (
                            <VideoView
                                ...
                                isAnimatingOut={isAnimatingOut}
                                onUseVideoForStory={handleUseVideoForStory}
                            />
                        )}
